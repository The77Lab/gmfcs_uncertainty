...
class DataSet_Regression(Dataset):
    def __init__(self, npy_file_path, root_dir, transform=None, data_augmentation = False, replicated = False):
        self.npy_data = np.load(npy_file_path)
        self.root_dir = root_dir
        self.transform = Preprocess_Module(False)
    ...
    def __getitem__(self, idx):
        ...
        if not self.replicated:
            if self.data_augmentation:
                ...
                if random.random() < 0.5:
                    data = augmentation.Shear(data)
                if random.random() < 0.5:
                    data = augmentation.Flip(data)
                if random.random() < 0.5:
                    data = augmentation.masking(data)
                data = data[0, :, :, :]
            data = pre_process(data, self.transform)
            # data2 = pre_process(data2, self.transform)
